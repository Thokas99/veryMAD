summary_fixture <- function() {
  data.frame(
    ...
    lower = -Inf, upper = Inf, direction = rep(c("upper", "both"), each = 3),
...
test_that("observation summaries report failures without treating missing as failure", {
  ...
  expect_equal(out$mad_qc_outlier, c(TRUE, TRUE, FALSE))
